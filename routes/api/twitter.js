...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    // fill the array
    // await Follows.findOne({ user_id: req.params.userid })
    //   .then(follows => {
    //       if(follows){
    //         follows.following.push({user_id: "87468747" , screen_name:"bushdf"},{user_id: "874648747" , screen_name:"bush4df"})
    //         follows.save((err) =>{
    //           if(err){
    //             console.log("Error"+err);
    //           } else{
    //             res.json({ msg: follows });
    //           }  
    //        }) 
    //       }
    //   })
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    await Follows.findOne({ user_id: req.params.id })
      .then(follows => {
        if (follows) {
          res.json({
            hasFollowings: follows.following.length !== 0,
            date: follows.following[follows.following.length - 1] === undefined ? "" :
              follows.following[follows.following.length - 1].date - Date.now(),
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    await Follows.findOne({ user_id: req.body.userid })
      .then(follows => {
        if (follows.following.length !== 0) {
          ...
          client.get('friendships/lookup', params, function (error, tweet, response) {
            if (!error && response.statusCode === 200) {
              ...
              // get those accts with "connections": ["Follws", "followedby"]
              arr.map((i) => {
                if (i.connections[1]) {
                  // get original data
...
router.post("/unfollow", requireAuth, asyncHandler(async (req, res, next) =>{
...
followUsers = (req, res, userArray, client) => {
  ...
  async.each(userz, function (user, callback) {
    var params = {
      user_id: user.user_id,
      follow: true
    }
    client.post('friendships/create', params, async function (error, tweet, response) {
      if (!error && response.statusCode === 200) {
        await Follows.findOne({ user_id: req.body.userid })
          .then(follows => {
            var userObj = {
          ...
          $(".js-face-pile-container")
            ...
            .map((i, el) => {
              ...
              const screenName = $(el)
                .attr("href")
                .split("/")[1];
              const photo = $(el).children("img").attr("src");
              const name = $(el).children("img").attr("alt")

              // users[`${screenName}`] = {userId: userId, photo: photo};
              // data.count += 1;
              users[count] = { name: name, screen_name: screenName, userId: userId, photo: photo }
              count++
            });
