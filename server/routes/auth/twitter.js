...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (error) {
      res.status(500).send({error:"Error getting OAuth request token : " + inspect(error)});
...
router.get(
  ...
  consumer.getOAuthAccessToken(req.params.oauthRequestToken, req.params.oauthRequestTokenSecret, req.params.oauth_verifier, function(error, oauthAccessToken, oauthAccessTokenSecret, results) {
    if (error) {
      res.status(500).send({error :"Error getting OAuth access token : " + inspect(error) + "[" + oauthAccessToken + "]" + "[" + oauthAccessTokenSecret + "]" + "[" + inspect(results) + "]"});
    } else {      
        consumer = null
